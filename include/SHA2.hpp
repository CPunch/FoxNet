...
// 08/21/21 - Added *_file implementations to hash file streams [cpunch]
...
inline sha256_hash
sha256_impl_file(const uint32_t* s, std::FILE *file, uint64_t length)
{
    static_assert(sizeof(uint32_t) == 4, "sizeof(uint32_t) must be 4");
    static_assert(sizeof(uint64_t) == 8, "sizeof(uint64_t) must be 8");
    ...
    constexpr size_t chunk_bytes = 64;
    const uint64_t bit_length = length * 8;
    uint8_t chunkBuf[chunk_bytes]; // copy chunks into this buffer to then work on
    ...
    while (length >= chunk_bytes) {
        // read a chunk from our file stream
        if (fread((void*)chunkBuf, sizeof(uint8_t), chunk_bytes, file) != chunk_bytes) {
            printf("SHA2: Failed to read from stream!\n");
            exit(EXIT_FAILURE);
        }
    ...
    {
        // read final bytes into chunk
        if (fread((void*)chunkBuf, sizeof(uint8_t), length, file) != length) {
            printf("SHA2: Failed to read from stream!\n");
            exit(EXIT_FAILURE);
        }
        ...
        write_u64(&chunkBuf[i], bit_length);
...
inline sha512_hash
sha512_impl_file(const uint64_t* s, std::FILE *file, uint64_t length)
{
    static_assert(sizeof(uint32_t) == 4, "sizeof(uint32_t) must be 4");
    static_assert(sizeof(uint64_t) == 8, "sizeof(uint64_t) must be 8");
...

    {
        ...
        write_u64(&chunkBuf[i + 0], bit_length_high);
        write_u64(&chunkBuf[i + 8], bit_length_low);
...
inline sha224_hash
sha224_file(std::FILE *file, uint64_t length)
{
    // Second 32 bits of the fractional parts of the square roots of the ninth
    // through sixteenth primes 23..53
    const uint32_t initial_hash_values[8] = {0xc1059ed8,
    ...
    auto hash = sha256_impl_file(initial_hash_values, file, length);
    return truncate<sha224_hash>(hash);
}
...
inline sha256_hash
sha256_file(std::FILE *file, uint64_t length)
{
    // First 32 bits of the fractional parts of the square roots of the first
    // eight primes 2..19:
    const uint32_t initial_hash_values[8] = {0x6a09e667,
    ...
    return sha256_impl_file(initial_hash_values, file, length);
}
...
inline sha384_hash
sha384_file(std::FILE *file, uint64_t length)
{
    const uint64_t initial_hash_values[8] = {0xcbbb9d5dc1059ed8,
                                             0x629a292a367cd507,
                                             0x9159015a3070dd17,
    ...
    auto hash = sha512_impl_file(initial_hash_values, file, length);
    return truncate<sha384_hash>(hash);
}
...
inline sha512_hash
sha512_file(std::FILE *file, uint64_t length)
{
    const uint64_t initial_hash_values[8] = {0x6a09e667f3bcc908,
                                             0xbb67ae8584caa73b,
                                             0x3c6ef372fe94f82b,
    ...
    return sha512_impl_file(initial_hash_values, file, length);
}
